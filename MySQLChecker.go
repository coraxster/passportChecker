...
func migrate(db *sql.DB) error {
	_, err := db.Exec("CREATE TABLE `value_store` (`val` varchar(16) NOT NULL DEFAULT '',`date_added` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),PRIMARY KEY (`val`),KEY `index_date` (`date_added`))")
	if err != nil {
		return err
	}
	return nil

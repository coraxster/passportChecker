...
// super slow, but with index, 30-50% cpu
...
func (c *SQLiteChecker) migrate() error {
	_, err := c.db.Exec("CREATE TABLE IF NOT EXISTS value_store (id INTEGER PRIMARY KEY, val TEXT UNIQUE, date_added INTEGER)")
	if err != nil {
		return err
	}
	_, err = c.db.Exec("CREATE INDEX IF NOT EXISTS date_index ON value_store (date_added);")
	if err != nil {
		return err
	}
	_, err = c.db.Exec("CREATE INDEX IF NOT EXISTS sn_index ON value_store (val);")
	if err != nil {
		return err
	}
	return nil
